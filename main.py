...
from telegram_client import TelegramAdminExtractor
...
def build_csv_rows(projects: list[ProjectListing], admin_map) -> list[dict]:
    rows: list[dict] = []
    for project in projects:
        for telegram_link in project.telegram_links:
            admins = admin_map.get(telegram_link) or []
            if not admins:
                rows.append(
                    {
                        "project_name": project.project_name,
                        "project_url": project.project_url,
                        "date_posted": project.date_posted.isoformat(),
    ...
    if telegram_links:
        async with TelegramAdminExtractor(settings) as extractor:
            for link in tqdm(telegram_links, desc="Telegram groups"):
                admin_map[link] = await extractor.extract_admins(link)

    rows = build_csv_rows(projects, admin_map)
